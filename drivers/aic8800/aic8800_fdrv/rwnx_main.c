...
void rwnx_insert_vendor_extension_in_bcn(struct rwnx_bcn *bcn) {
  ...
  memcpy(
      &temp_ie[index + 1], vendor_extension_data,
      vendor_extension_len /*sizeof(vendor_extension_data)*/); // insert vendor
                                                               // extension data
  ...
  memcpy(bcn->tail, temp_ie,
         bcn->tail_len +
             vendor_extension_len /*sizeof(vendor_extension_data)*/);
  bcn->tail_len =
      bcn->tail_len + vendor_extension_len /*sizeof(vendor_extension_data)*/;
  ...
  if (ctxt->count == 0) {
    if (vif->ch_index == vif->rwnx_hw->cur_chanctx) {
      ...
      rwnx_radar_detection_enable(&vif->rwnx_hw->radar,
                                  RWNX_RADAR_DETECT_DISABLE, RWNX_RADAR_RIU);
    }
...
/* struct net_bridge	*br = netdev->br_port->br; */ /* ->dev->dev_addr; */
...
#endif
    {
      ...
      if (br_netdev) {
        ...
        dev_put(br_netdev);
        AICWFDBG(LOGINFO,
                 FUNC_NDEV_FMT " bind bridge dev " NDEV_FMT "(" MAC_FMT ")\n",
                 FUNC_NDEV_ARG(netdev), NDEV_ARG(br_netdev),
                 MAC_ARG(br_netdev->dev_addr));
      } else {
        AICWFDBG(LOGINFO,
                 FUNC_NDEV_FMT " can't get bridge dev by name \"%s\"\n",
...
static int rwnx_open(struct net_device *dev) {
  ...
  if (rwnx_hw->vif_started == 0) {
    ...
    if (rwnx_hw->usbdev->chipid == PRODUCT_ID_AIC8800DC ||
      ...
      error = rwnx_send_dbg_mem_mask_write_req(rwnx_hw, 0x4033b300, 0xFF,
                                               rwnx_rx_gain);
      if (error) {
        return error;
      }
    ...
    if (!rwnx_hw->is_p2p_alive) {
      if (rwnx_hw->p2p_dev_vif && !rwnx_hw->p2p_dev_vif->up) {
        err = rwnx_send_add_if(rwnx_hw, rwnx_hw->p2p_dev_vif->wdev.address,
                               RWNX_VIF_TYPE(rwnx_hw->p2p_dev_vif), false,
                               &add_if_cfm);
        if (err) {
          return -EIO;
        }
  ...
  if (RWNX_VIF_TYPE(rwnx_vif) == NL80211_IFTYPE_AP_VLAN) {
    ...
    if ((error =
             rwnx_send_add_if(rwnx_hw, rwnx_vif->wdev.address,
  ...
  if (RWNX_VIF_TYPE(rwnx_vif) == NL80211_IFTYPE_MONITOR) {
    ...
    if (rwnx_vif->ch_index != RWNX_CH_NOT_SET) {
      ...
      error = rwnx_send_config_monitor_req(
          rwnx_hw, &rwnx_hw->chanctx_table[rwnx_vif->ch_index].chan_def, NULL);
    }
#if defined(CONFIG_RWNX_MON_XMIT)
    rwnx_txq_unk_vif_init(rwnx_vif);
#endif
#if defined(CONFIG_RWNX_MON_RXFILTER)
    rwnx_send_set_filter(rwnx_hw, (FIF_BCN_PRBRESP_PROMISC | FIF_OTHER_BSS |
...
static int rwnx_close(struct net_device *dev) {
  ...
  while (atomic_read(&rwnx_vif->drv_conn_state) ==
    ...
    AICWFDBG(LOGDEBUG,
             "%s wifi is connecting or disconnecting, waiting 200ms for state "
             "to stable\r\n",
             __func__);
    msleep(200);
    test_counter--;
  ...
  if (netif_carrier_ok(dev)) {
    if (RWNX_VIF_TYPE(rwnx_vif) == NL80211_IFTYPE_STATION ||
        RWNX_VIF_TYPE(rwnx_vif) == NL80211_IFTYPE_P2P_CLIENT) {
      cfg80211_disconnected(dev, WLAN_REASON_DEAUTH_LEAVING, NULL, 0, true,
                            GFP_ATOMIC);
      netif_tx_stop_all_queues(dev);
      netif_carrier_off(dev);
...
#endif
{
  ...
  // printk("%s enter %d \r\n", __func__,
  // atomic_read(&rwnx_hw->p2p_alive_timer_count));
 ...
 * Cfg80211 callbacks (and helper)
 *********************************************************************/
static struct wireless_dev *rwnx_virtual_interface_add(
    struct rwnx_hw *rwnx_hw, const char *name, unsigned char name_assign_type,
      ...
      if (is_broadcast_ether_addr(addr) || is_multicast_ether_addr(addr)) {
        ...
        list_for_each_entry(rwnx_sta, &rwnx_vif->ap.sta_list, list) {
          AICWFDBG(LOGDEBUG,
                   ...
                   __func__, rwnx_sta->mac_addr[0], rwnx_sta->mac_addr[1],
                   rwnx_sta->mac_addr[2], rwnx_sta->mac_addr[3],
                   rwnx_sta->mac_addr[4], rwnx_sta->mac_addr[5], addr[0],
                   addr[1], addr[2], addr[3], addr[4], addr[5]);
          if (rwnx_sta->valid && ether_addr_equal(rwnx_sta->mac_addr, addr)) {
...
static int rwnx_cfg80211_change_iface(struct wiphy *wiphy,
  ...
  if ((type == NL80211_IFTYPE_MONITOR) &&
    ...
    list_for_each_entry(vif_el, &rwnx_hw->vifs, list) {
      ...
      if ((vif_el != vif) && (RWNX_VIF_TYPE(vif) != NL80211_IFTYPE_MONITOR)) {
        wiphy_err(rwnx_hw->wiphy,
                  "Monitor+Data interface support (MON_DATA) disabled\n");
        return -EIO;
      }
...
#endif
#if defined(CONFIG_RWNX_MON_RXFILTER)
    rwnx_send_set_filter(
        vif->rwnx_hw,
        (FIF_BCN_PRBRESP_PROMISC | FIF_OTHER_BSS | FIF_PSPOLL | FIF_PROBE_REQ));
#endif
  } else {
...
int rwnx_send_check_p2p(struct cfg80211_scan_request *param) {
  ...
  for (i = 0; i < index; i++) {
    if (!memcmp("DIRECT-", param->ssids[i].ssid, (sizeof("DIRECT-") - 1))) {
...
static int rwnx_cfg80211_connect(struct wiphy *wiphy, struct net_device *dev,
  ...
  /* Forward the information to the LMAC */
  if ((error =
           rwnx_send_sm_connect_req(rwnx_hw, rwnx_vif, sme, &sm_connect_cfm)))
    return error;
  ...
  case CO_OK: {
    ...
    if (atomic_read(&rwnx_hw->sta_flowctrl[sta->sta_idx].tx_pending_cnt) > 0)
      AICWFDBG(
          ...
          atomic_read(&rwnx_hw->sta_flowctrl[sta->sta_idx].tx_pending_cnt));
...
#define PRINT_STA_FLAG(f)                                                      \
  (params->sta_flags_set & BIT(NL80211_STA_FLAG_##f) ? "[" #f "]" : "")
    ...
    netdev_info(dev, "Add sta %d (%pM) flags=%s%s%s%s%s%s%s", sta->sta_idx, mac,
                PRINT_STA_FLAG(AUTHORIZED), PRINT_STA_FLAG(SHORT_PREAMBLE),
                PRINT_STA_FLAG(WME), PRINT_STA_FLAG(MFP),
                PRINT_STA_FLAG(AUTHENTICATED), PRINT_STA_FLAG(TDLS_PEER),
...
void apm_staloss_work_process(struct work_struct *work) {
  ...
  if (found) {
    ...
    // if (rwnx_vif->wdev.iftype == NL80211_IFTYPE_AP || rwnx_vif->wdev.iftype
    // == NL80211_IFTYPE_P2P_GO) {
    //		cfg80211_del_sta(rwnx_vif->ndev, cur->mac_addr, GFP_KERNEL);
    //	}
    ...
    if ((rwnx_vif->wdev.iftype == NL80211_IFTYPE_STATION) ||
      ...
      list_for_each_entry_safe(reord_info, reord_tmp, &rx_priv->stas_reord_list,
                               list) {
        AICWFDBG(LOGINFO, "reord_mac:%x,%x,%x,%x,%x,%x\r\n",
                 reord_info->mac_addr[0], reord_info->mac_addr[1],
                 reord_info->mac_addr[2], reord_info->mac_addr[3],
...
void apm_probe_sta_work_process(struct work_struct *work) {
  ...
  if (found)
    cfg80211_probe_status(rwnx_vif->ndev, mac,
                          (u64)rwnx_vif->sta_probe.probe_id, 1, 0, false,
                          GFP_ATOMIC);
  else
    cfg80211_probe_status(rwnx_vif->ndev, mac,
                          (u64)rwnx_vif->sta_probe.probe_id, 0, 0, false,
                          GFP_ATOMIC);
#else
  if (found)
    cfg80211_probe_status(rwnx_vif->ndev, mac,
                          (u64)rwnx_vif->sta_probe.probe_id, 1, GFP_ATOMIC);
  else
    cfg80211_probe_status(rwnx_vif->ndev, mac,
                          (u64)rwnx_vif->sta_probe.probe_id, 0, GFP_ATOMIC);
#endif
...
#undef PRINT_STA_FLAG
  ...
  if (params->vlan) {
    ...
    if (sta->vlan_idx != vlan_idx) {
      ...
      if ((RWNX_VIF_TYPE(vif) == NL80211_IFTYPE_AP_VLAN) && (vif->use_4addr)) {
...
#endif
  // Forward the information to the LMAC
  error = rwnx_send_bcn_change(rwnx_hw, vif->vif_index, elem.dma_addr, bcn->len,
  ...
  if (rwnx_vif == rwnx_hw->p2p_dev_vif && !rwnx_vif->up) {
    ...
    if (!rwnx_hw->is_p2p_alive) {
      error = rwnx_send_add_if(
          rwnx_hw, rwnx_vif->wdev.address, // wdev->netdev->dev_addr,
          RWNX_VIF_TYPE(rwnx_vif), false, &add_if_cfm);
      if (error)
        return -EIO;
  ...
  if (rwnx_survey->filled != 0) {
    ...
    info->noise =
        ((IS_2P4GHZ(info->channel->center_freq)) ? DEFAULT_NOISE_FLOOR_2GHZ
         : (IS_5GHZ(info->channel->center_freq)) ? DEFAULT_NOISE_FLOOR_5GHZ
...
// size_t len = params->len;
  ...
  AICWFDBG(LOGDEBUG, "%s rwnx_sta:%p RWNX_VIF_TYPE(rwnx_vif):%d \r\n", __func__,
           rwnx_sta, RWNX_VIF_TYPE(rwnx_vif));
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 8, 0))
    error = rwnx_cfg80211_remain_on_channel_(wiphy, wdev, channel, 30, &cookie,
                                             true);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3, 8, 0)) &&                        \
    (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 6, 0))
    error = rwnx_cfg80211_remain_on_channel_(
        wiphy, wdev, channel, NL80211_CHAN_NO_HT, 30, &cookie, true);
#else
    error = rwnx_cfg80211_remain_on_channel_(
        wiphy, dev, channel, NL80211_CHAN_NO_HT, 30, &cookie, true);
#endif
...
#else
  return rwnx_start_mgmt_xmit(rwnx_vif, rwnx_sta, channel, offchan, wait, buf,
...
static int rwnx_fill_station_info(struct rwnx_sta *sta, struct rwnx_vif *vif,
  ...
  AICWFDBG(LOGDEBUG,
           "%s ModTx(%d):%d TxIndex:%d ModRx(%d):%d RxHTIndex:%d RxVHTIndex:%d "
           "RxHEIndex:%d RSSI:%d \r\n",
           __func__, rate_info->bwTx, rate_info->formatModTx,
           rate_info->mcsIndexTx, rx_vect1->ch_bw, rx_vect1->format_mod,
           rx_vect1->ht.mcs, rx_vect1->vht.mcs, rx_vect1->he.mcs, (s8)cfm.rssi);
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 0, 0)
  sinfo->filled |=
      (STATION_INFO_INACTIVE_TIME | STATION_INFO_RX_BYTES64 |
       STATION_INFO_TX_BYTES64 | STATION_INFO_RX_PACKETS |
       STATION_INFO_TX_PACKETS | STATION_INFO_SIGNAL | STATION_INFO_RX_BITRATE);
#else
  sinfo->filled |=
      (BIT(NL80211_STA_INFO_INACTIVE_TIME) | BIT(NL80211_STA_INFO_RX_BYTES64) |
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 0, 0)
  sinfo->filled =
      (STATION_INFO_LLID | STATION_INFO_PLID | STATION_INFO_PLINK_STATE |
       STATION_INFO_LOCAL_PM | STATION_INFO_PEER_PM | STATION_INFO_NONPEER_PM);
#else
  sinfo->filled =
 ...
 * (invoked with the wireless_dev mutex held)
 */
static int rwnx_cfg80211_leave_mesh(struct wiphy *wiphy,
                                    struct net_device *dev) {
  struct rwnx_hw *rwnx_hw = wiphy_priv(wiphy);
  struct rwnx_vif *rwnx_vif = netdev_priv(dev);
...
u32 syscfg_tbl_u04[][2] = {
    {0x40040000, 0x0000042C}, // protect usb replenish rxq / flush rxq, skip
                              // flush rxq before start_app
...
static void system_config_8800(struct rwnx_hw *rwnx_hw) {
  ...
  if (rwnx_hw->usbdev->chipid == PRODUCT_ID_AIC8801) {
    if (chip_id == CHIP_REV_U02) {
      ...
      for (cnt = 0; cnt < syscfg_num; cnt++) {
        ret = rwnx_send_dbg_mem_write_req(rwnx_hw, syscfg_tbl_pmic_u02[cnt][0],
  ...
  if (rwnx_hw->usbdev->chipid == PRODUCT_ID_AIC8801) {
    if ((chip_id == CHIP_REV_U03) && (chip_sub_id == CHIP_SUB_REV_U04)) {
      ...
      for (cnt = 0; cnt < syscfg_num; cnt++) {
        ret = rwnx_send_dbg_mem_write_req(rwnx_hw, syscfg_tbl_u04[cnt][0],
                                          syscfg_tbl_u04[cnt][1]);
        if (ret) {
          AICWFDBG(LOGERROR, "%x write fail: %d\n", syscfg_tbl_u04[cnt][0],
                   ret);
          return;
        }
      }
  ...
  for (cnt = 0; cnt < syscfg_num; cnt++) {
    ...
    ret = rwnx_send_dbg_mem_mask_write_req(rwnx_hw, syscfg_tbl_masked[cnt][0],
                                           syscfg_tbl_masked[cnt][1],
                                           syscfg_tbl_masked[cnt][2]);
    if (ret) {
      AICWFDBG(LOGERROR, "%x mask write fail: %d\n", syscfg_tbl_masked[cnt][0],
               ret);
      return;
    }
  }
...
static void aicwf_usb_config(struct rwnx_hw *rwnx_hw) {
  ...
  if ((rd_mem_addr_cfm.memdata & 0xffff) == 0x119) {
    ...
    for (cnt = 0; cnt < usbcfg_num; cnt++) {
      ret = rwnx_send_dbg_mem_write_req(rwnx_hw, usbcfg_tbl[cnt][0],
                                        usbcfg_tbl[cnt][1]);
      if (ret) {
...
// #ifndef CONFIG_RFTEST
  ...
  if (rwnx_hw->usbdev->chipid == PRODUCT_ID_AIC8800DC ||
      rwnx_hw->usbdev->chipid == PRODUCT_ID_AIC8800DW) {
    ret = rwnx_send_set_stack_start_req(rwnx_hw, 1, 0, 0, rwnx_hw->fwlog_en,
                                        &set_start_cfm);
  } else {
  ...
  if (rwnx_hw->usbdev->chipid == PRODUCT_ID_AIC8800D81) {
    ...
    ret = rwnx_send_set_stack_start_req(rwnx_hw, 1, get_hardware_info(),
                                        feature.hwinfo, rwnx_hw->fwlog_en,
                                        &set_start_cfm);
  }
#endif
  ...
  memcpy(wiphy->fw_version, fw_version.fw_version,
         fw_version.fw_version_len > 32 ? 32 : fw_version.fw_version_len > 32);
  ...
  *platform_data = rwnx_hw;
  ...
  /* Add an initial station interface */
  vif = rwnx_interface_add(rwnx_hw, "wlan%d", 1, NL80211_IFTYPE_MONITOR, NULL);
#endif
...
static int __init rwnx_mod_init(void) {
  ...
  if ((wait_for_completion_timeout(&hostif_register_done,
                                   msecs_to_jiffies(REGISTRATION_TIMEOUT)) ==
       0)) {
